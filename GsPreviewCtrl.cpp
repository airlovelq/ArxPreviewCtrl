...
extern CAcExtensionModule ArxPreviewCtrlDLL;			//BlockView2002DLL;
...
#ifdef _WIN64
	::SetClassLong(m_hWnd, GCLP_HCURSOR, NULL);
#else
...
Acad::ErrorStatus CGsPreviewCtrl::InitDrawingControl( const TCHAR *space )
{
	...
	if (es == Acad::eOk)
	{
		...
		if (es == Acad::eOk)
		{
			...
			// 初始化控件资源
			Init(ArxPreviewCtrlDLL.ModuleResourceInstance(), true);
...
void CGsPreviewCtrl::SetFitView(AcGsView *pView,AcDbExtents ext)
{
	// 计算图形数据库模型空间的大小
	AcGePoint3d extMax = ext.maxPoint();
	AcGePoint3d extMin = ext.minPoint();
...
void CGsPreviewCtrl::AppendTempEntity(AcDbEntity *pEnt)
{
	AcGsView *pView = view();
	pView->add(pEnt,model());
	AcDbExtents ext = GetModelSpaceExtent(m_pDb);
	AcDbExtents ex;
	pEnt->getGeomExtents(ex);
	ext.addExt(ex);
	SetFitView(pView,ext);
}